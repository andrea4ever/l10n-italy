...
# ------------------------------------------------------------------------------
class FiscalYearClosingInit(models.Model):
    """
...
class FiscalYearClosingLpAccountMapping(models.Model):
    """
...
class FiscalYearClosingNlpAccountMapping(models.Model):
    """
...
class FiscalYearClosingCAccountMapping(models.Model):
    """
...
class FiscalYearClosing(models.Model):
        ...
        for source, dest, description in mapping:
            ...
            if source_account_id:
                ...
                if dest_account_id:
                    ...
                    account_mappings.append({
                        'name': _(
                            'No destination account {dest} '
                            'found for account {source}.'.format(
    ...
    def action_draft(self, cr, uid, ids, context=None):
        ...
        for fyc in self.browse(cr, uid, ids, context):
            ...
            if len(fyc_ids) > 1:
                raise UserError(
                    _('There is already a fiscal year closing '
                      'with this name.'))
            ...
            # have to be computed or they have
            # already been computed (restarted workflow)
            #
    ...
    def action_run(self, cr, uid, ids, context=None):
        ...
        """
        # Note: Just change the state, everything else is done
        # on the run wizard
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for fyc in self.browse(cr, uid, ids, context):
            #
            # Require the L&P, closing, and opening
            # moves to exist (NL&P is optional)
            #
